<?php 
require("../../../libs/config.php");
$title = "JavaScript Program for Writing Content to child window";
$description = "Write a JavaScript program for writing content to child window.";
include($_SERVER['DOCUMENT_ROOT'].SITE_DIR."include/header.php");
?>

<?php 
include($_SERVER['DOCUMENT_ROOT'].SITE_DIR."include/navbar.php");
?>

<!-- Breadcrumb -->
<div class="breadcrumbs overlay">
  <div class="container">
    <div class="row">
      <div class="col-12">
        <div class="bread-inner">
          <!-- Bread Menu -->
          <div class="bread-menu">
            <ul>
              <li><a href="<?php echo SITE_DIR;?>">Home</a></li>
              <li><a href="<?php echo SITE_DIR;?>javascript-programming/">JavaScript Programming</a></li>
			  <li><a href="<?php echo SITE_DIR;?>javascript-programming/programs/">All Programs</a></li>
            </ul>
          </div>
          <!-- Bread Title -->
          <div class="bread-title"><h2>JavaScript Program for Writing Content to child window</h2></div>
        </div>
      </div>
    </div>
  </div>
</div>
<!--/ End Breadcrumb -->

<section class="news-area archive blog-single section-padding">
	<div class="container">
		<div class="row">
		
<?php 
include($_SERVER['DOCUMENT_ROOT'].SITE_DIR."include/leftbar.php");
?>
	
	<div class="col-sm-8">
		<div class="blog-single-main">
			<div class="blog-detail">
				<h2 class="blog-title">JavaScript Program for Writing Content to child window</h2>
				<p>Write a JavaScript program for writing content to child window.</p>
				
				<br>
				<?php 
					include($_SERVER['DOCUMENT_ROOT'].SITE_DIR."include/banner-ads.php");
				?>
				<br>
				<h5>Source Code</h5>
				<div id="code" class="source-code">
					<pre><code class="html">&lt;html&gt;
&lt;head&gt;
    &lt;title&gt;Writing to child window using JS&lt;/title&gt;
    &lt;script&gt;
        function addContent()
        {
            window.topPage.document.open();
            window.topPage.document.write('&lt;html&gt;');
            window.topPage.document.write('&lt;body&gt;');
            window.topPage.document.write('&lt;form action=""&gt;');
            window.topPage.document.write('&lt;input type="text"/&gt;');
            window.topPage.document.write('&lt;input type="button" value="submit"/&gt;');
            window.topPage.document.write('&lt;/form&gt;');
            window.topPage.document.write('&lt;/body&gt;');
            window.topPage.document.write('&lt;/html&gt;');
            window.topPage.document.close();
        }
    &lt;/script&gt;
&lt;/head&gt;
&lt;frameset rows="25%, *" onload="addContent()"&gt;
    &lt;frame name="topPage"/&gt;
    &lt;frame src="webpage1.html" name="bottomPage"/&gt;
&lt;/frameset&gt;
&lt;/html&gt;


&lt;html&gt;
&lt;head&gt;
    &lt;title&gt;Webpage 1&lt;/title&gt;
&lt;/head&gt;
&lt;body&gt;
&lt;h1&gt;Webpage 1&lt;/h1&gt;
&lt;/body&gt;
&lt;/html&gt;


&lt;html&gt;
&lt;head&gt;
    &lt;title&gt;Webpage 2&lt;/title&gt;
&lt;/head&gt;
&lt;body&gt;
&lt;img name="targetImage" alt="No image selected"/&gt;
&lt;/body&gt;
&lt;/html&gt;
</code></pre>
				</div> <!--/ End source-code -->
				</div> <!--/ End blog-detail-->
				
			</div> <!--/ End blog-single-main -->
		

				<h5 class="pt-3">Output</h5>
				<div class="output">
					<pre><code class="plaintext"><img src="write-content-to-child-window.jpg"></code></pre>
				</div>
				<!-- DESC -->

			<br>
      
			<?php 
				include($_SERVER['DOCUMENT_ROOT'].SITE_DIR."include/feedback.php");
			?>

			<br>
			<?php 
				include($_SERVER['DOCUMENT_ROOT'].SITE_DIR."include/banner-ads.php");
			?>
			<br>
		</div> <!-- /col-sm-8 -->	
<?php 
include$_SERVER['DOCUMENT_ROOT'].SITE_DIR."include/rightbar.php";
?>
	</div>	<!-- /row -->

</div> <!-- /container -->
 
<?php 
include($_SERVER['DOCUMENT_ROOT'].SITE_DIR."include/footer.php");
?>